...
def predict_and_rescale(
    predictor_model: Regressor,
    target_encoder: CustomTargetEncoder,
    transformed_data: pd.DataFrame,
):
    """
    Make predictions with the regressor and then inverse transform the predictions

    Args:
        predictor_model (Regressor): The regressor model.
        target_encoder (CustomTargetEncoder): The target encoder.
        transformed_data (pd.DataFrame): The preprocessed input data for prediction.

    Returns:
        np.ndarray: The predicted targets.
    """
    predictions_arr = predict_with_model(predictor_model, transformed_data)
    rescaled_predictions_arr = inverse_transform_targets(
        target_encoder, predictions_arr
    )
    return rescaled_predictions_arr
...
def create_predictions_dataframe(
    ...
    """
    predictions_df = pd.DataFrame(predictions_arr, columns=[prediction_field_name])
